...
<!-- AUTO:STATE -->
<!-- Généré automatiquement par scripts/v9_sync_state.py — 2026-07-28 11:43 UTC -->
<!-- Ne pas éditer manuellement. Pour forcer : python scripts/v9_sync_state.py -->

| Métrique | Valeur | Source |
|---|---|---|
| HEAD | `57058da chore(v9): resync strategy_overrides post UnifiedMetaLearningLoop cycle 28/07` | `git log --oneline -1` |
...
| Crons Ready | 36 | `Get-ScheduledTask (PowerShell)` |
...
**HEAD** : `3b2c6fd` (5 commits session : b906c15 → 3b2c6fd)

**5 causes racines décalage paper trade vs réel corrigées** :
- CAUSE 1 : DRM adaptatif TP/SL dans resolver (phase de cycle au lieu de 10/10 statique)
- CAUSE 2 : Horizon resolver 4h→8h (93% time_end → 2.6%)
- CAUSE 3 : V9_NO_BAISSIERE=0 + V9_GBPUSD_LONG_ONLY=0 (2 directions)
- CAUSE 4 : Colonnes bayésiennes persistées dans signals table (confiance_calibree + 5 predictor_*)
- CAUSE 5 : Resolver filtre paires blacklistées (USDCAD, AUDUSD, USDJPY)

**Replay 9059 décisions** : WR 17%→71%, +6.2 pips/trade, tp_hit 67%

**Boucle d'apprentissage fermée** : learn_loop n_enter 94%, WR uplift +1.81pts, Brier 0.258→0.203, Walk-forward EDGE RÉEL (OOS 5.979p, 4/4 folds)

**Phase E activée** : DRAWDOWN_PROTECTOR, RISK_PARITY, CYCLE_MEMORY, CROSS_PAIR_METRICS, WALK_FORWARD tous ON

**Purge DB** : 17.42 GB→10.49 GB (-6.93 GB)

**Principes** : 56 total (41 ACTIVE + 15 SHADOW). PRINCIPLE_ACTIVE_IDS aligné sur YAML.
