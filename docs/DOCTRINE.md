...
| 12 | Replay et live sont marqués distinctement dans les décisions | Résolu le 2026-07-06 : colonne `source_type` ("live"/"replay") ajoutée aux 8 tables dérivées (scenes, behaviors, windows, exploitability, regime_snapshots, principle_evaluations, signals, decisions), peuplée par `orchestrator.run_chain(source_type=...)`. `"live"` par défaut (capture temps réel), `"replay"` passé par `regenerate_chain.py`. Voir `docs/checkpoints/CHECKPOINT_20260706_V9_SOURCE_TYPE.md`. |
