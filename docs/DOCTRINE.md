...
| **25'** | **Vocabulaire descriptif : les principes `kind: grammar` sont du vocabulaire descriptif de lecture, pas des hypothèses de rentabilité à valider statistiquement. Promotion SHADOW → ACTIVE conditionnée à la maturité structurelle (conditions réellement écrites + champs contexte PROPAGÉS + décision Søn tracée) — jamais à un hit_rate arbitraire** | **Remplace R25** ("hit_rate >= 60%"), supprimée pour contradiction 🔴 avec CHARTE Interdit #4 + Règle 3 (`docs/audit/AUDIT_DOCTRINE_REPORT.md` §3.1/§3.2). Décision actée `workspace/perplexity/memory/DECISIONS_LOG.md` 2026-07-08. N'interdit pas l'observation a posteriori du win/loss par principe (règle 30) — ce qui est interdit, c'est de conditionner la promotion au hit_rate, pas de l'observer. |
