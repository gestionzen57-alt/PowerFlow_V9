...
commit `78d2621`, plus code Phase 9 non commité en cours sur session concurrente).
...
data/v9_forces.db → table forces_snapshots
    ...
    5. RegimeDetector        → table regime_snapshots        [Phase 9, en cours]
    6. PrincipleEngine       → tables principles / principle_evaluations [Phase 9, en cours]
    7. SignalGenerator       → table signals                 [Phase 9, en cours]
    8. DecisionLogger        → table decisions               [Phase 9, en cours]
```
...
6. Décision (Phase 9, en cours) — transforme en action recommandée, jamais un ordre direct
...
## Modules `core/v9/` — stables (Phases 1-8)
...
## Modules `core/v9/` — Phase 9, en cours (session concurrente, non commité)

> Documenté en lecture seule à titre d'inventaire. Ce code appartient à une session en
> cours ailleurs ; ne pas le modifier depuis cette branche. Détail complet et à jour dans
> [docs/phases/PHASE9_DECISION.md](phases/PHASE9_DECISION.md), à finaliser à la clôture.

| Module | Rôle (état constaté au 2026-07-05) |
|---|---|
| `orchestrator.py` | `run_chain(snapshot_id)` — étend la chaîne Forces→Exploitabilité avec Régime→Principes→Signal→Décision, chaque étape isolée par try/except |
| `regime_db.py` / `regime_detector.py` | `RegimeDetector` — machine à états portée de V8, régimes NEUTRE/PALIER/CASSURE/EXTENSION/RETOUR_EQUILIBRE/REJET sur fenêtre glissante |
| `principle_db.py` / `principle_engine.py` | `PrincipleEngine` — évalue les principes YAML (`core/v9/principles/`), 10 principes ACTIVE routés, 17 en mode SHADOW (journalisés, non routés) |
| `signal_db.py` / `signal_generator.py` | `SignalGenerator` — agrège les principes ACTIVE déclenchés en direction + confiance, filtré par exploitabilité et régime |
| `decision_db.py` / `decision_logger.py` | `DecisionLogger` — action recommandée qualitative (observer/surveiller/preparer_entree/aucune_action), jamais un ordre |
| `zone_db.py` | Schéma `zone_diagnostics` — table créée, **non encore alimentée** par un détecteur (gap connu, hors scope Phase 9) |
| `core/v9/principles/*.yaml` | 27 définitions de principes (9 `node_rule` + 18 `GRAMMAR_*`), portés de V8 après audit |
