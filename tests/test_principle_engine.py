...
def test_v9_status_split_25_active_28_shadow():
    ...
    2026-07-15 (session ZCode) : 5 principes mis DORMANT (COALITION_NODE,
    NODE_BIRTH_FAST, RAW_NODE_BIRTH, ELASTIC_BREATH, GRAMMAR_CONTEXTE) +
    3 _ADAPTIVE promus ACTIVE (GRAMMAR_CONTEXTE_ADAPTIVE WR 79.5%,
    POWER_ANGLE_BREAK_TO_PRICE_IMPACT_ADAPTIVE WR 75.0%, ZONE_RETEST_ADAPTIVE
    ...
    assert len(active) == 47, f"attendu 47 ACTIVE (27/07 promotion flash), got {len(active)} : {[p.principle_id for p in active]}"
    assert len(shadow) == 9, f"attendu 9 SHADOW (27/07 promotion flash), got {len(shadow)} : {[p.principle_id for p in shadow]}"
    ...
    # 27/07 promotion flash: POWER_ANGLE_BREAK_TO_PRICE_IMPACT_ADAPTIVE promu ACTIVE
    ...
    expected_shadow = {
        ...
        "VOLUME_CONFIRMATION",
        # 22/07 recalibrage — principes perdants démodulés (restent SHADOW) :
        "ZONE_RETEST",
        "ZONE_RETEST_ADAPTIVE",
        # 27/07 promotion flash : ADAPTIVE_VOL_GATE, VELOCITY_CLIMAX_GUARD,
        # POWER_ANGLE_BREAK_TO_PRICE_IMPACT, PRICE_LAG_AT_NODE_BIRTH
        # et leurs _ADAPTIVE promus ACTIVE
    }
...
def test_engine_syncs_principles_table(db_path: Path):
    ...
    2026-07-15 (session ZCode) : 5 principes mis DORMANT + 3 _ADAPTIVE
    promus ACTIVE (GRAMMAR_CONTEXTE_ADAPTIVE, POWER_ANGLE_BREAK_TO_PRICE_IMPACT_ADAPTIVE,
    ZONE_RETEST_ADAPTIVE). Le statut v9_status dérive de PRINCIPLE_ACTIVE_IDS :
    25 ACTIVE, les 28 autres tombent en SHADOW via cette API.
    """
    ...
    assert n_active == 47, f"attendu 47 ACTIVE (27/07 promotion flash), got {n_active}"
