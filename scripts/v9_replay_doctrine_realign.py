...
pré-patch (10 principes ACTIVE historiques, ensemble figé ci-dessous) vs
post-patch (config.py actuel) — Phase C6 doctrine realign
(§R8-levée-doctrine-realign, DECISIONS_LOG 2026-07-08). Utilisé pour la
calibration Phase D (COMPARAISON_DOCTRINE_REPLAY.md, 0 régression). La
promotion C1 (10→27 ACTIVE) a été rejetée en Phase E (clôture CEO) :
POST_PATCH_ACTIVE_IDS == PRINCIPLE_ACTIVE_IDS == PRE_PATCH_ACTIVE_IDS
aujourd'hui (10 éléments identiques des deux côtés) ; ce script reste
disponible pour rejouer une future proposition de promotion.
...
# indépendant de config.PRINCIPLE_ACTIVE_IDS. La promotion C1 (10→27) a
# été rejetée en Phase E : config.PRINCIPLE_ACTIVE_IDS est identique à
# cet ensemble aujourd'hui, mais ce script reste utile pour rejouer une
# future proposition de promotion contre l'état pré-patch figé ici.
