...
# Phase 14 (CEO autopilot 2026-07-15) — singleton Lazy pour
# LearningOffsetApplier (lit `learning_proposals` table à chaque consolidate()
# mais ne crée pas d'état mutable hormis la connexion DB).
_LEARNING_OFFSET_APPLIER: LearningOffsetApplier | None = None


def _get_learning_offset_applier() -> LearningOffsetApplier:
    global _LEARNING_OFFSET_APPLIER
    if _LEARNING_OFFSET_APPLIER is None:
        _LEARNING_OFFSET_APPLIER = LearningOffsetApplier()
    return _LEARNING_OFFSET_APPLIER
...
class Arbiter:
    ...
    def _compute_learning_offset_multiplier(
        ...
        [0.85, 1.15]). Idempotent, lecture seule, R6 : toute erreur ->
        (1.0, 'neutral'). R25' : kill switch `V9_LEARNING_OFFSET_ENABLED`
        OFF par défaut.

        Anti-bridage : n'instancie pas de DB (le singleton
        `LearningOffsetApplier` ouvre/ferme sa propre connexion interne).
        """
        try:
        ...
        try:
            ...
            if not rows:
                ...
                return {
                    ...
                    "learning_offset_multiplier": LEARNING_OFFSET_MULT_NEUTRAL,
                    "learning_offset_basis": "neutral",
            ...
            # Phase 14 (CEO autopilot 2026-07-15) — pondération learning_offset
            # (propositions APPROVED du learning_loop). Insérée APRÈS Brief Q1
            # pour être la dernière pondération AVANT la zone R29 (qui opère
            # ensuite sur la confiance finale). Kill switch OFF par défaut
            # (V9_LEARNING_OFFSET_ENABLED=0, R25' strict). Bornes [0.85, 1.15],
            # magnitude limitée. Lecture seule sur `learning_proposals` table,
            # fallback neutre si R6 violation. Jamais None.
            (
                learning_offset_multiplier,
            ...
            if learning_offset_multiplier != LEARNING_OFFSET_MULT_NEUTRAL:
                ...
                confiance_finale = round(
                    min(
                        100,
                        max(0, confiance_finale * learning_offset_multiplier),
                    )
                )
                plafonne = plafonne or (confiance_finale != confiance_avant_p14)
            ...
            return {
                ...
                "learning_offset_multiplier": learning_offset_multiplier,
                "learning_offset_basis": learning_offset_basis,
                "learning_offset_direction": direction_majoritaire
                if learning_offset_basis == "approved"
                else None,
                "timestamp": ts_max,
                "arbiter_version": ARBITER_VERSION,
                "snapshot_id": snapshot_id,
